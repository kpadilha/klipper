...
class ProbeEndstopWrapper:
    def __init__(self, config, mcu_endstop):
        ...
        self.activate_gcode = config.get('activate_gcode', "").split('\n')
        self.deactivate_gcode = config.get('deactivate_gcode', "").split('\n')
        # Wrappers
    ...
    def home_prepare(self):
        self.gcode.process_commands(self.activate_gcode, need_ack=False)
        self.mcu_endstop.home_prepare()
    def home_finalize(self):
        self.gcode.process_commands(self.deactivate_gcode, need_ack=False)
...
class ProbePointsHelper:
    def __init__(self, printer, probe_points, horizontal_move_z, speed,
        ...
        if not manual_probe:
            while self.busy:
                self.gcode.process_commands(["PROBE"], need_ack=False)
                self.cmd_NEXT({})
    def move_next(self):
